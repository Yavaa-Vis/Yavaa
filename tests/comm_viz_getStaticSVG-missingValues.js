 ...
 *
 * - includes a fraction of missing values 
 * 
 * stores the results in /tests/vizResults with suffix _missing
 *
 */
define( [ 'module',
          'basic/Constants',
  ...
  return function( QUnit ) {
    ...
    const MISSING_FRACTION = 0.2;

    QUnit.module( 'viz/*: datasets including missing values' );
    ...
    function createTest( vizName, bindingNr ) {
      ...
      QUnit.test( `${vizName} (binding ${bindingNr}) getStaticSVG()`, async function( assert ){
        ...
        try {
          ...
          desc[ bindingNr ].testing = desc[ bindingNr ].testing || {};
          desc[ bindingNr ].testing.missing = MISSING_FRACTION;
          ...
          const Fs    = require( 'fs' ),
                Path  = require( 'path' ),
                path  = Path.join( requirejs.toUrl(''), '..', '..', 'tests', 'vizResults', `${vizName}${bindingNr}_missing.svg` );
