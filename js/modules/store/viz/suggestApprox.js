 ...
 * * appoximate edition: all dimensions must be bound + at least one measurement
 */
define([
  'viz/RepoList',
  'basic/Constants',
  ...
  async function suggestApprox( ds ) {
    ...
    // create candidates
    // single viz
    let candidates = vizList.filter( function( binding ){
      return (binding[ DESC.COLCOUNT ] <= colCount)                                       // only some columns match
    ...
    return {
      suggestion:   feasBindings,
      omittedCols:  colOmitted.map( (c) => c.getID() ),
    };
  ...
  function findBinding( cols, bindings ) {
    ...
    for( let i=0; i<matching.length; i++ ) {
      ...
      // memorize columns that were used in a binding
      usedCols.add( mapCols[ matching[i][0] ] );
    ...
    // all dimensions must be bound
    for( const col of cols ) {
      if( col.isDimension() && !usedCols.has(col) ) {
        return null;
      }
    }

    // at least one measurement must be bound
    if( !cols.some( (col) => !col.isDimension() && usedCols.has(col) ) ) {
      return null;
    }
